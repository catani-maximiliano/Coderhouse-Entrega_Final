...
//Funcion de consumo de api de recetas
async function apiRespuesta(a) {
  ...
  urlAPI = `https://api.edamam.com/api/recipes/v2?type=public&q=${buscadorReceta || a}&app_id=${app_id}&app_key=%20${app_key}`;
  ...
  // creacion de las tarjetas de recetas
  for (const receta of data.hits) {
    let health = [];
    let l = receta.recipe.healthLabels.length;
    let punto = "&#9679";

    // los healthlabels son las etiquetas como sin gluten, etc...
      ...
      <div class="card-body ">
        ...
        <div class="d-flex col-md-12 d-flex justify-content-between align-content-center row my-2 mx-auto ">
          <div class="col-md-4 d-flex justify-content-center mx-auto row">
            <small class="text-muted">comida tipica: ${
...
//boton de siguiente, donde muestra mas resultados
function buttonNext(ab) {
  urlAPI = ab;
  apiRespuesta();
}
...
//ordena los favoritos por las calorias
...
//ordena los favoritos por el nombre de receta
...
//busqueda de receta de favorito por nombre
function busquedaReceta() {
  if (usuarioConectadoJson === true) {
    let recetaB = document.getElementById("formRecetaBusqueda").value;
    recetaB = recetaB[0].toUpperCase() + recetaB.substr(1);

    favoritoJson = JSON.parse(localStorage.getItem("favorito"));
    recetaBa = [];

    document.getElementById("boton-cerrar-busqueda").click();
    document.getElementById("boton-cerrar-busqueda").value = "";
...
//borra las recetas de favoritos
...
//Funciones de registro, login y cerrar sesion
function Login() {
